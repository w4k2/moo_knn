...
for data_idx, dataset in enumerate(datasets):
    ...
    if len(classes) == 2 and counts[0]/counts[1] > 2.5:
        ...
        ax.scatter(scores_weighted[:,0], scores_weighted[:,1], marker="s", c="cornflowerblue")
        ax.scatter(scores_bac[0], scores_bac[1], marker="*", c="gold")
        ax.scatter(scores_ref[:,0], scores_ref[:,1], marker="x", c="tomato")
        ...
        fig.legend(["weighted k-NN", "genetic BAC", "sklearn k-NN"])
        plt.tight_layout()
