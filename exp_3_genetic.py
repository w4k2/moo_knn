...
for data_idx, dataset in enumerate(datasets):
    ...
    if len(classes) == 2 and counts[0]/counts[1] > 2.5:
        ...
        for fold, (train, test) in enumerate(rskf.split(X, y)):
            ...
            problem = KnnOneCrit(X[train], y[train])
            ...
            np.save(f"results/genetic/opt_variables_{fold}_{dataset}", res.X)
            np.save(f"results/genetic/opt_metrics_{fold}_{dataset}", res.F)
